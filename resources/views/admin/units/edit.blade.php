                    ...
                    @foreach($unit->images as $img)
                        <label class="text-center">
                            <img src="{{ asset('storage/'.$img->image_url) }}" alt=""
                                 class="w-[120px] h-[90px] object-cover rounded-lg border border-gray-200">
                            <div class="flex items-center justify-center gap-1 mt-1">
                                <input class="rounded border-gray-300" type="checkbox" name="delete_images[]" value="{{ $img->image_id }}" id="del{{ $img->image_id }}">
                                <label class="text-xs text-gray-600" for="del{{ $img->image_id }}">حذف</label>
                            </div>
                        </label>
                    @endforeach
                </div>
