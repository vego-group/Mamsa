  ...
  <form id="unitForm"
    ...
    <div>
      <div class="card">
        ...
        @endif
	{{-- ===== بيانات التصريح (أفراد) ===== --}}
      	@if($type === 'individual')
        	<label class="label">رقم تصريح وزارة السياحة</label>
        	<input class="input"
               	type="text"
               	name="tourism_permit_no"
               	required>

        	<label class="label mt-2">ملف التصريح السياحي (PDF)</label>
        	<input class="input"
               	type="file"
               	name="tourism_permit_file"
               	accept="application/pdf"
               	required>
                <div class="mt-3 bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm">
          	<b>   الهوية</b>
          	{{ auth()->user()->AdminDetails->national_id }}
        	</div>
      	@endif

      	{{-- ===== بيانات التصريح (شركات) ===== --}}
      	@if($type === 'company')
        	...
        	<label class="label mt-2">ملف الترخيص (PDF)</label>
        	<input class="input"
               	type="file"
               	name="tourism_permit_file"
               	accept="application/pdf"
        	...
        	{{-- عرض CR فقط --}}
        	<div class="mt-3 bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm">
          	<b>رقم السجل التجاري (CR):</b>
          	{{ auth()->user()->AdminDetails->cr_number }}
        	</div>
      	@endif


          <label class="label">اسم العقار</label>
         <input class="input" type="text" name="unit_name" id="name" required>

          <div class="row-2">
            <div>
              <label class="label">نوع العقار</label>
             <select class="select" name="unit_type" id="type">
...
{{-- ===== مميزات الوحدة ===== --}}
<label class="block mb-4 text-lg font-semibold text-[#2f4b46] text-right">
    مميزات الوحدة
</label>
...
@foreach($features as $feature)
    <label class="cursor-pointer">
        ...
        <input
            ...
            value="{{ $feature->id }}"
            class="peer hidden"
            @checked(isset($unit) && $unit->features->contains($feature->id))
        >
        ...
        <div class="
            ...
            peer-checked:bg-[#294c45]
            peer-checked:text-white
            peer-checked:border-[#294c45]
            hover:shadow-sm
        ">
            {{ $feature->name }}
        </div>
...
{{-- ===== سياسة الإلغاء ===== --}}
<label class="label">سياسة الإلغاء</label>
<select name="cancellation_policy" class="select">
    <option value="">— اختر السياسة —</option>
    <option value="no_cancel">غير قابلة للإلغاء</option>
