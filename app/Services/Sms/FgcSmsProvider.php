...
class FgcSmsProvider implements SmsProvider
{
    ...
    // OTP رسائل خدمية → messageTypeId = 1
    ...
    {
        // Token صالح دقيقة واحدة فقط — نجلبه مع كل إرسال
        ...
        // E001 = success، أي مفتاح آخر = error code
    ...
    private function authenticate(): string
    {
        ...
        if (! $response->successful() || blank($response->json('token'))) {
            ...
            throw new \RuntimeException('FGC SMS: فشل التحقق من الهوية');
        }
